...
def load_speedup_csv(csv_path: str) -> tuple[list[int], dict[str, list[float]]]:
    ...
    for col in header[1:]:
        ...
        if label.endswith("-threads"):
            label = label[:-8]
        numeric_token = label.split("-")[0]
        threads.append(int(numeric_token))

    # Restrict to desired thread counts if present (e.g., CSV may include 16-thread data)
    desired_threads = {2, 4, 8}
    selected_indices = [i for i, t in enumerate(threads) if t in desired_threads]
    if not selected_indices:
        raise ValueError("CSV must contain columns for 2, 4, or 8 threads")
    threads = [threads[i] for i in selected_indices]
    ...
    for row in rows[1:]:
        ...
        raw_values = [float(x) for x in row[1:]]
        values = [raw_values[i] for i in selected_indices]
