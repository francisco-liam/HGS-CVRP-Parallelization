...
{
	eval = EvalIndiv();
	chromRWithChargers.assign(params.nbVehicles, std::vector<int>());
	for (int r = 0; r < params.nbVehicles; r++)
	{
		if (!chromR[r].empty())
		{
			// Insert charging stations and compute distance with chargers
			FRVCP::RouteResult routeResult = FRVCP::buildRouteWithChargers(params, chromR[r]);
			chromRWithChargers[r] = routeResult.routeWithChargers;
			if (!routeResult.feasible)
			{
				eval.energyExcess = 1.0;
			}
			// Distance already includes depot legs and chargers
			double distance = routeResult.distance;
	...
	eval.penalizedCost = eval.distance
		...
		+ eval.energyExcess*params.penaltyEnergy;
	eval.isFeasible = (eval.capacityExcess < MY_EPSILON && eval.durationExcess < MY_EPSILON && eval.energyExcess < MY_EPSILON);
}
...
Individual::Individual(Params & params)
{
	...
	chromRWithChargers = std::vector < std::vector <int> >(params.nbVehicles);
